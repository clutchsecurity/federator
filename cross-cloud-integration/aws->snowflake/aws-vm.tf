...
# AWS IAM Role for EC2 instances, allowing them to request web identity tokens for Snowflake federation.
...
resource "aws_instance" "demo" {
  depends_on = [
    ...
    snowflake_execute.wif_service_user,
  ]
  provider               = aws.ec2-region
  ...
  user_data = templatefile("${path.module}/tpl/aws-vm.tpl", {
    ...
    snowflake_username  = var.snowflake_wif_username,
  })
  tags = {
